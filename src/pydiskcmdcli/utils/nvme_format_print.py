...
def format_print_ocp_info_check(decoded_data, print_type='normal'):
    if print_type == 'normal':
        if decoded_data["ocp_info"]["support"]:
            print ("Device %s support OCP(spec ver %s) command" % (decoded_data["device"]["dev_path"],'.'.join([str(i) if isinstance(i, int) else 'x' for i in decoded_data["ocp_info"]["version"]])))
        else:
...
def format_print_ocp_latency_monitor_log(cmd, print_type='normal'):
    raw_data = cmd.data
    if print_type == 'normal' or print_type == 'json':
        decoded_data = ocp_latency_monitor_decode(raw_data)
        for k in list(decoded_data.keys()):
            decoded_data[k] = scsi_ba_to_int(decoded_data[k], 'little')
        if print_type == 'normal':
            print ("OCP Error Recovery -")
