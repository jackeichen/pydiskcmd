...
def check_nvme_if_ctrl_path(dev_path: str):
    return (dev_path.startswith("/dev/") and get_nvme_device_type(dev_path) == 0)

def get_nvme_ctrl_ids() -> List:
    """
    Return: a list that contain ctrl_id, like ["nvme0", "nvme1"]
    """
    if os.path.exists('/sys/class/nvme'):
        return sorted([dev for dev in os.listdir('/sys/class/nvme') 
                       if get_nvme_device_type(dev) == 0])
    return []

def get_nvme_device_type(name: str):
    '''
    Return:
...
def get_block_devs(print_detail: bool = False,
    ...
    if exclude:
        excluded_block_devices += exclude
...
def get_nvme_block_dev_by_ctrl(ctrl: str) -> List:
    """
    Return: a list that contain ns_id under ctrl_id, like ["nvme0n1", "nvme0n2"]
    """
    if get_nvme_device_type(ctrl) == 0:
        path = os.path.join('/sys/class/nvme', ctrl)
...
class NVMeController(object):
    ...
    @property
    def numa_node(self):
        return self._get_str_by_file('numa_node')

    @property
    def queue_count(self):
        return self._get_str_by_file('queue_count')

    @property
    def sqsize(self):
        return self._get_str_by_file('sqsize')
...
def scan_nvme_ctrls() -> Dict:
    ctrl_names = get_nvme_ctrl_ids()
    all_info = {}
    for i in ctrl_names:
