...
class ATACommand12(ATAPassThrough12):
    ...
    def check_return_status(self, success_hint=False, fail_hint=True, raise_if_fail=True) -> bool:
        ...
        if decode_sense and decode_sense.data.get("sense_key") in (0x01,  # Recovered Error
            ...
            _success = False  # get error here
            # except return asc/ascq=0/0x1d, this is used to notify useful data in sense data.
            # TODO: Linux usually return sense_key = 0x01, as well as windows 10. But still need
            # test more windows version, before enable checking sense_key
            # if decode_sense.data.get("sense_key") == 1 and decode_sense.asc == 0 and decode_sense.ascq == 29:
            if decode_sense.asc == 0 and decode_sense.ascq == 29:
                _success = True
